...
class VisionProcessor:
    ...
    def __init__(self, model_name: str = None):
        ...
        """
        # Vision 모델은 별도로 설정 (일반 LLM과 다를 수 있음)
        vision_model = "gemini-2.0-flash-exp"  # Vision 지원 모델
    ...
    def convert_page_to_image(self, pdf_path: str, page_num: int, dpi: int = 200) -> Optional[Image.Image]:
        ...
        try:
            ...
            # 고화질 이미지로 변환 (DPI 200 이상)
            zoom = dpi / 72.0  # PyMuPDF는 72 DPI 기준
    ...
    def convert_page_to_markdown(self, pdf_path: str, page_num: int, max_retries: int = 3) -> Optional[str]:
        ...
        [작동 원리]
        1. PDF 페이지를 고화질 이미지(DPI 200)로 변환
        2. Gemini Vision 모델에 이미지와 시스템 프롬프트 전송
        3. 표는 <table_summary> 태그로 요약 포함
        4. 완벽한 Markdown 형식으로 변환
...
5. 표가 아닌 일반 텍스트는 줄바꿈을 정리하여 자연스럽게 이어지도록 하라."""
        ...
        # 3단계: Gemini Vision 호출 (재시도 로직 포함)
    ...
    def convert_section_to_markdown(self, pdf_path: str, start_page: int, end_page: int, max_workers: int = 4) -> list:
        ...
        - ThreadPoolExecutor로 최대 max_workers개 페이지 동시 처리
        - API 호출 시간을 최대한 활용하여 전체 처리 시간 단축
        ...
        Returns:
            [(page_num, markdown_text), ...] 리스트 (페이지 번호 순서대로 정렬)
        """
        results = []
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_page):
                ...
                try:
                    ...
                    if markdown:
                        results.append((page_num + 1, markdown))  # 1-based로 저장
        ...
        # 페이지 번호 순서대로 정렬 (병렬 처리로 인한 순서 보장)
